...
export type XunleiOnResolveHandler = (ctx: OnResolveContext) => Promise<void> | void

export type XunleiOnStartHandler = (ctx: OnStartContext) => Promise<void> | void

export type XunleiOnErrorHandler = (ctx: OnErrorContext) => Promise<void> | void
...
export interface WebviewPage {
  ...
  /** 向指定元素输入文本。 */
  type(selector: string, text: string, opts?: WebviewTypeOptions): Promise<void>
  /** 等待选择器匹配；超时时返回 false。 */
  waitForSelector(selector: string, opts?: WebviewWaitForSelectorOptions): Promise<boolean>
  /** 等待脚本返回真值；超时时返回 null。 */
...
export interface XunleiHostUi {
  ...
  pickDirectory(opts?: UiPickDirectoryOptions): Promise<UiPickDirectoryResult | null>
  /** 跳转到宿主任务列表页，可选高亮指定任务。 */
