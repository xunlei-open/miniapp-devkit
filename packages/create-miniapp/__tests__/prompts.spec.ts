...
function mockPromptFlow() {
  ...
  mockSelect.mockResolvedValueOnce('vanilla').mockResolvedValueOnce('typescript').mockResolvedValueOnce(false)
...
test('skips linter selection when user declines linter', async () => {
  ...
  expect(mockSelect).not.toHaveBeenCalledWith(expect.objectContaining({ message: 'Select linter / formatter:' }))
})
