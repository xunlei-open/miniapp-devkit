...
test('loads an image and updates the crop, keyboard position and preview reactively', async () => {
  expect(wrapper.get<HTMLButtonElement>('#download').element.disabled).toBe(true)
  await chooseImage()
  expect(wrapper.get('#meta').text()).toContain('photo.png · 800 × 600 px')
  expect(wrapper.get<HTMLButtonElement>('#download').element.disabled).toBe(false)
  ...
  await wrapper.get('#crop').trigger('keydown', { key: 'ArrowRight', shiftKey: true })
  ...
  expect(drawImage).toHaveBeenLastCalledWith(expect.anything(), 110, 0, 300, 300, 0, 0, 300, 300)
...
async function pointer(element: Element, name: string, init: PointerEventInit) {
  element.dispatchEvent(new window.PointerEvent(name, { ...init, bubbles: true, cancelable: true }))
  await nextTick()
}
...
test('accepts a dropped image and moves and resizes the crop with pointer events', async () => {
  ...
  vi.spyOn(wrapper.get('#stage').element, 'getBoundingClientRect').mockReturnValue({
    width: 400,
    height: 300,
  } as DOMRect)
