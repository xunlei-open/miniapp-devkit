...
export default function miniapp(options: MiniappOptions = {}): Plugin {
  ...
  return {
    ...
    config(config) {
      ...
      const manifest = eventsOnly
        ? undefined
        : (JSON.parse(readFileSync(resolve(projectRoot, manifestFile), 'utf8')) as
            { entry?: { type?: string; url: string } } | undefined)
      const page = manifest?.entry && (manifest.entry.type ?? 'miniapp') === 'miniapp' ? manifest.entry.url : undefined
    ...
    closeBundle() {
      ...
      copyFileSync(resolve(projectRoot, manifestFile), resolve(projectRoot, outDir, 'manifest.json'))
