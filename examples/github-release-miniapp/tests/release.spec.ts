...
test.each([
  ...
  'https://github.com/cli/cli/releases/latest?x=1#assets',
...
test.each([
  '',
  'cli',
  'https://evil.test/cli/cli',
  'https://github.com.evil.test/cli/cli',
  'https://github.com@evil.test/cli/cli',
  'https://github.com/cli/cli/releases/download/v1/a.zip',
  'https://github.com/cli/cli/archive/refs/tags/v1.zip',
...
test('extracts and deduplicates Assets, source archives and attestations with safe filenames', () => {
  ...
  expect(assets.map((asset) => asset.name)).toEqual([
    ...
    'source-v1.zip',
    'source-v1.tar.gz',
    'attestation-123.json',
  ])
  expect(assets.every((asset) => asset.url.startsWith('https://github.com/cli/cli/'))).toBe(true)
  const unsafe = parseAssets(
    '<a href="/cli/cli/releases/download/v1/..%2Fbad.exe">x</a>',
...
test('follows the lazy Assets fragment, not unrelated includes or release-note links', async () => {
  const fetcher = vi
    ...
    .mockResolvedValueOnce(
      response(
        `<include-fragment src="https://evil.test/cli/cli/releases/expanded_assets/v1"></include-fragment>
      <include-fragment src="/cli/cli/releases/expanded_assets/v1"></include-fragment>
      <a href="/cli/cli/releases/download/old/old.zip">Release notes</a>`,
  ...
  expect(fetcher.mock.calls.map((call) => call[0])).toEqual([
    'https://github.com/cli/cli/releases/latest',
    'https://github.com/cli/cli/releases/expanded_assets/v1',
  ])
  expect(release.assets).toHaveLength(5)
  expect(releaseResource(release).files[0]).toEqual({
    name: 'app.zip',
    path: '',
    size: 0,
...
test('supports inline assets and reports empty, missing and restricted releases', async () => {
  ...
  await expect(resolveRelease('cli/cli', vi.fn().mockResolvedValue(new Response('<html></html>')))).rejects.toThrow(
    '未找到 Assets',
  )
  for (const status of [403, 404, 429, 500]) {
    await expect(resolveRelease('cli/cli', vi.fn().mockResolvedValue(new Response('', { status })))).rejects.toThrow(
      String(status),
    )
  }
})
...
test('event shares the parser and leaves download links untouched', async () => {
  ...
  vi.stubGlobal('xunlei', {
    events: {
      onResolve: (value: typeof handler) => {
        handler = value
      },
    },
  })
