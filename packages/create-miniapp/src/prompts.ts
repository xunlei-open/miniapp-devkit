...
export async function runPrompts(defaults: Partial<CreateOptions> = {}): Promise<CreateOptions> {
  ...
  const features = supportsOptionalFeatures(framework)
    ? await promptFeatures(framework, variant, defaults.features ?? defaultFeatures(framework))
    : []

  const packageManager = defaults.packageManager ?? detectInvokedPackageManager()
...
async function promptFeatures(framework: Framework, _variant: Variant, defaultSelected: Feature[]): Promise<Feature[]> {
...
async function promptLintTool(framework: Framework, defaultSelected: Feature[]): Promise<'lint' | 'biome'> {
  const biomeLabel = isBiomeExperimental(framework) ? 'Biome (lint + format) (Experimental)' : 'Biome (lint + format)'
