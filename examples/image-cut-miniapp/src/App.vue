...
async function load(file?: File) {
  ...
  try {
    ...
    if (img.naturalWidth * img.naturalHeight > 40_000_000 || Math.max(img.naturalWidth, img.naturalHeight) > 16384) {
      throw new Error('图片尺寸过大，请使用不超过 4000 万像素、单边不超过 16384 像素的图片。')
    }
    endDrag()
    ...
    status(error instanceof Error ? error.message : '图片读取失败，请重新选择。', true)
  }
}
...
function changeDimension(key: keyof Rect, event: Event) {
  const input = event.target as HTMLInputElement
  state.rect = updateCrop(state.rect, key, Number(input.value), state.width, state.height, state.ratio)
...
function moveWithKeyboard(e: KeyboardEvent) {
  if (!['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown'].includes(e.key) || state.busy) return
  e.preventDefault()
  const step = e.shiftKey ? 10 : 1
  state.rect.x = clamp(
    state.rect.x + (e.key === 'ArrowRight' ? step : e.key === 'ArrowLeft' ? -step : 0),
    0,
    state.width - state.rect.w,
  )
  state.rect.y = clamp(
    state.rect.y + (e.key === 'ArrowDown' ? step : e.key === 'ArrowUp' ? -step : 0),
    0,
...
async function download() {
  ...
  try {
    ...
    ctx.drawImage(source, state.rect.x, state.rect.y, state.rect.w, state.rect.h, 0, 0, state.rect.w, state.rect.h)
    const blob = await new Promise<Blob>((resolve, reject) =>
      canvas.toBlob((b) => (b ? resolve(b) : reject(new Error('图片编码失败，请缩小裁剪区域。'))), mime, 0.92),
    )
    canvas.width = 1
    canvas.height = 1
    ...
    status(`已创建下载任务：${name}（${task.id}）。请在迅雷查看进度，下载完成前保持应用打开。`)
  } catch (error) {
    if (hostURL) {
      try {
...
<template>
  <header>
    <div class="brand">
      <span class="logo">◩</span><span>迅雷图片裁剪<small>IMAGE CUT</small></span>
    </div>
    ...
    <div class="workspace">
      <section class="editor panel" aria-label="图片裁剪编辑器">
        <div class="panel-title">
          ...
          <button id="choose" :disabled="state.busy" @click="fileInput?.click()" class="secondary">选择图片</button
          ><input
            id="file"
            ref="fileInput"
            @change="chooseFile"
        ...
        >
          ...
          >
            <img id="source" :src="state.imageURL || undefined" alt="待裁剪的原图" draggable="false" />
            <div
              id="crop"
              ref="crop"
              :style="cropStyle"
            ...
            >
              ...
              ><span class="handle nw" data-corner="nw"></span><span class="handle ne" data-corner="ne"></span
              ><span class="handle sw" data-corner="sw"></span><span class="handle se" data-corner="se"></span>
            </div>
          </div>
        </div>
        <div class="image-meta">
          <span id="meta">{{ state.meta }}</span
          ><button id="reset" class="text-button" :disabled="!state.width || state.busy" @click="reset">
            重置裁剪
          </button>
        </div>
        <div class="preview-row">
          <div class="preview-block">
            <div class="preview-heading">
              <label>实时预览</label
              ><span id="size">{{ state.width ? `${state.rect.w} × ${state.rect.h} px` : '—' }}</span>
            </div>
            <div class="preview">
              <canvas id="preview" ref="preview" width="1" height="1" aria-label="裁剪结果预览"></canvas
              ><span id="preview-empty" :hidden="!!state.width">裁剪结果将在这里显示</span>
            </div>
          </div>
          <p class="hint">拖动选框移动，拖动四角调整大小。也可使用像素输入精确裁剪。</p>
        </div>
      </section>
      <aside class="panel settings">
        ...
        <fieldset id="controls" :disabled="!state.width || state.busy">
          ...
          ><input id="filename" v-model="state.filename" type="text" maxlength="100" placeholder="裁剪图片" />
        </fieldset>
        <button id="download" class="primary" :disabled="!state.width || state.busy" @click="download">
          ↓ 通过迅雷下载
        </button>
        <p id="status" :class="{ error: state.error }" role="status" aria-live="polite">
          {{ state.message }}
        </p>
      </aside>
    </div>
