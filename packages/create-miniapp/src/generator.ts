...
import { hasFeature, pmDevCommand, pmInstallCommand, pmTestCommand, resolveTemplateId } from './utils.js'
...
function shouldSkip(relPath: string, options: CreateOptions): boolean {
  ...
  if (!hasFeature(features, 'lint')) {
    if (normalized.endsWith('eslint.config.js') || normalized.endsWith('eslint.config.js.ejs')) {
      return true
    }
  ...
  if (!hasFeature(features, 'biome')) {
    ...
    if (normalized === '.vscode/settings.json' || normalized.endsWith('/.vscode/settings.json')) {
      return true
    }
  }
  if (relPath.endsWith('pnpm-lock.yaml') || relPath.endsWith('package-lock.json') || relPath.endsWith('yarn.lock')) {
    return true
  }
  return false
}

async function renderFile(src: string, dest: string, context: TemplateContext): Promise<void> {
...
async function copyTemplateDir(
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      await copyTemplateDir(srcPath, path.join(targetDir, entry.name), context, options, relPath)
      continue
    }
...
export async function generateProject(options: CreateOptions): Promise<string> {
  ...
  if (!(await fs.pathExists(templateDir))) {
    throw new Error(`Template "${templateId}" is not available (${options.framework} + ${options.variant}).`)
  }
