  ...
  .action(async (projectName: string | undefined, opts) => {
    try {
      ...
      if (frameworkInput) {
        ...
        if (!parsed) {
          throw new Error(`Unknown framework: ${frameworkInput} (supported: vue | react | vanilla)`)
        }
      ...
      if (variantInput) {
        ...
        if (!parsed) {
          throw new Error(`Unknown variant: ${variantInput} (supported: typescript | javascript)`)
        }
      ...
      if (packageManagerInput) {
        ...
        if (!parsed) {
          throw new Error(`Unknown package manager: ${packageManagerInput} (supported: npm | yarn | pnpm)`)
        }
      ...
      console.log(cyan(`\n ${CLI_NAME} \n`))
      const options = opts.yes ? resolveOptionsFromDefaults(defaults) : await runPrompts(defaults)
...
function resolveOptionsFromDefaults(defaults: Partial<CreateOptions>): CreateOptions {
