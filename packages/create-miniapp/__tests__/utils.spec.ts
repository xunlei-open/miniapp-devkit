...
test('toPackageName', () => {
  ...
  expect(toPackageName('XUNLEI-MINIAPP')).toBe('xunlei-miniapp')
  expect(toPackageName('xunlei-   miniapp    1.0.0')).toBe('xunlei-miniapp-1-0-0')
  expect(toPackageName('xunlei---miniapp')).toBe('xunlei-miniapp')
...
test('detectInvokedPackageManager', () => {
  vi.stubEnv('npm_config_user_agent', 'pnpm/9.15.0 npm/? node/v22.21.1 darwin arm64')
  expect(detectInvokedPackageManager()).toBe('pnpm')
  vi.stubEnv('npm_config_user_agent', 'npm/10.9.4 node/v22.21.1 darwin arm64 workspaces/false')
  expect(detectInvokedPackageManager()).toBe('npm')
  vi.stubEnv('npm_config_user_agent', 'yarn/1.22.22 npm/? node/v22.21.1 darwin arm64')
...
test('resolveMutuallyExclusiveLint', () => {
  expect(resolveMutuallyExclusiveLint(['lint', 'biome'])).toStrictEqual(['lint'])
  expect(resolveMutuallyExclusiveLint(['lint', 'biome', 'vitest'])).toStrictEqual(['lint', 'vitest'])
  expect(resolveMutuallyExclusiveLint(['biome', 'vitest'])).toStrictEqual(['biome', 'vitest'])
})
...
test('normalizeFeatures', () => {
  expect(normalizeFeatures(['lint', 'biome'], 'vanilla')).toStrictEqual(['lint'])
