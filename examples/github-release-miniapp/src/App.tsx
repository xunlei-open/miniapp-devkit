  ...
  return <main>
    ...
    <form onSubmit={resolve}>
      ...
      <div className="input-row">
        <input id="repository" value={input} onChange={event => setInput(event.target.value)}
          placeholder="cli/cli 或 https://github.com/cli/cli" required disabled={busy} />
        <button type="submit" disabled={busy}>解析最新发布</button>
      </div>
    </form>
    <p id="status" role="status" aria-live="polite">{message}</p>
    {release && <section aria-label="Release Assets">
      <div className="toolbar">
        <label><input type="checkbox" disabled={busy}
          checked={selected.size === release.assets.length}
          ref={element => { if (element) element.indeterminate = selected.size > 0 && selected.size < release.assets.length }}
          onChange={event => setSelected(new Set(event.target.checked ? release.assets.map(asset => asset.url) : []))} /> 全选</label>
        <h2>{release.repository}</h2>
      </div>
      <ul>{release.assets.map(asset => <li key={asset.url}>
        <label><input type="checkbox" aria-label={asset.name} disabled={busy} checked={selected.has(asset.url)}
          onChange={event => {
          ...
          <span className="asset-size">{asset.sizeText ?? '大小未知'}</span></label>
      </li>)}</ul>
      <footer><span>已选择 {selected.size} / {release.assets.length} 个文件</span>
        <button type="button" disabled={busy || !selected.size} onClick={download}>下载所选</button>
      </footer>
    </section>}
