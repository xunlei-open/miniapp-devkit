...
test.each(['./events/onStart.js', 'events/./onStart.js', 'events/nested/../onStart.js'])(
  'matches the normalized manifest entry %s',
  async (entry) => {
    ...
    for (const invalid of ['./events/missing.js', '../events/onStart.js']) {
      await expect(publish(new Map([['events/onStart.js', 'bad']]), entries, [invalid])).rejects.toThrow(
        'has no built event entry',
      )
      expect(await readFile(join(root, 'events/onStart.js'), 'utf8')).toBe('good')
    }
  },
...
test('publishes dependencies outside events and retains old dependencies after rebuild', async () => {
  ...
  await publish(
    new Map([
      ['events/onStart.js', 'first'],
      ['assets/a.js', 'old worker'],
    ]),
    entries,
    required,
  )
  await publish(
    new Map([
      ['events/onStart.js', 'second'],
      ['assets/b.wasm', 'new wasm'],
    ]),
    entries,
    required,
  )
...
test('validates the complete build before changing an existing entry', async () => {
  ...
  await expect(publish(new Map([['assets/new.js', 'new']]), new Set(), required)).rejects.toThrow(
    'has no built event entry',
  )
